...
class Datalink:
    ...
    def __init__(self, host: str = None, port: int = -1, timeout: float = -1, debug_mode: bool = False, max_incommming_messages_in_queue: int = -1) -> None:
        if port <= 0:
        ...
        if host is None:
            self.link = Datalink.lib.init_tcp_server(
                ctypes.c_int(port), ctypes.c_float(timeout), debug_mode, max_incommming_messages_in_queue)
        else:
            self.link = Datalink.lib.init_tcp_client(ctypes.c_char_p(
                host.encode('utf-8')), ctypes.c_int(port), ctypes.c_float(timeout), debug_mode, max_incommming_messages_in_queue)
    ...
    def setup_cpp_lib(cls) -> None:
        ...
        Datalink.lib.init_tcp_server.argtypes = [
            ctypes.c_int, 
            ctypes.c_float, 
            ctypes.c_bool,
            ctypes.c_int    # max_incommming_messages_in_queue
        ]

        Datalink.lib.init_tcp_client.restype = ctypes.c_void_p
        Datalink.lib.init_tcp_client.argtypes = [
            ctypes.c_char_p, 
            ctypes.c_int, 
            ctypes.c_float, 
            ctypes.c_bool,
    ...
    def read_bytes(self) -> tuple[bytes, int, float]:
        ...
        read_size = Datalink.lib.read_next_message(
            self.link, data_pointer, raw_size, ctypes.byref(timestamp))
    ...
    def read_np(self, shape, dtype) -> tuple[np.ndarray, int, float]:
        ...
        if size != expected_size:
            print(
                f"Data size does not match expected size: {size} != {expected_size}")
            return np.array([]), 0, 0
    ...
    def send_object(self, object: any) -> bool:
        ...
        while not acked:
            ...
            failed_ack_wait = False
            while not failed_ack_wait and (not self.is_ready() or not self.has_data()):
                if self._timeout > 0 and 1000*(time.time() - start) > self._timeout:
                    return False
                time.sleep(0.001)
                wait_loops += 1
            ...
            if not failed_ack_wait:
                ack, s, _ = self.read_bytes()
                acked = s == 1 and int(ack[0]) == 1
